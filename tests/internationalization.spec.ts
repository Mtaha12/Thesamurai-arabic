...
test.describe('Internationalization - Cross-Browser Tests', () => {
  ...
  test('should handle locale switching', async ({ page }) => {
    ...
    const arVisible = await arLink.isVisible({ timeout: 5000 }).catch(() => false);
    const enVisible = await enLink.isVisible({ timeout: 5000 }).catch(() => false);
  ...
  test('should handle invalid locale gracefully', async ({ page }) => {
    const response = await page.goto('/invalid-locale', { timeout: 30000 });
    await page.waitForLoadState('domcontentloaded', { timeout: 10000 }).catch(() => {});
    
    // Should redirect to valid locale or show 404
    const finalUrl = page.url();
    expect(finalUrl).toMatch(/\/(en|ar)|404|invalid/);
  });
});
